...
processArgs :: [String] -> (K, Size, InitSegSize, MaxHedgeDist, KeepCCZ, RunPreprocessing, PartAlg, PartDir, Format, SaveTrace, Verbose)
processArgs []     = (-1, -1, 1000, 100, False, True, Kahypar, "./", GateCount, False, True) -- Default values
processArgs (a:as) = case take 3 a of
    "--h" -> error $  "\n\n\nThis is a list of all available options. Use cat <circuit_file> | ./Main [options].\n"++
                      ...
                      "\n\n"
    "-k=" -> (read $ drop 3 a, s, w, m, kT, pp, alg, dir, o, sT, vb)
    "-s=" -> (k, read $ drop 3 a, w, m, kT, pp, alg, dir, o, sT, vb)
    "-w=" -> (k, s, read $ drop 3 a, m, kT, pp, alg, dir, o, sT, vb)
    "-m=" -> (k, s, w, read $ drop 3 a, kT, pp, alg, dir, o, sT, vb)
    "-cc" -> (k, s, w, m, True, pp, alg, dir, o, sT, vb)
    "-np" -> (k, s, w, m, kT, False, alg, dir, o, sT, vb)
    "-fp" -> (k, s, w, m, kT, pp, Patoh, dir, o, sT, vb)
    "-d=" -> (k, s, w, m, kT, pp, alg, drop 3 a, o, sT, vb)
    "-o=" -> case find (\(tag,_) -> tag == drop 3 a) format_enum of
      Just f -> (k, s, w, m, kT, pp, alg, dir, snd f, sT, vb)
      Nothing -> error $ "Option "++a++" not recognised."
    "-st" -> (k, s, w, m, kT, pp, alg, dir, o, True, vb)
    "-vb" -> (k, s, w, m, kT, pp, alg, dir, o, sT, False)
    _     -> error $ "Option "++a++" not recognised."
  where
    (k,s,w,m,kT,pp,alg,dir,o,sT,vb) = processArgs as
...
main = do
  ...
  let
    (cfg_k,cfg_s,cfg_initSegSize,cfg_maxHedgeDist,cfg_keepCCZ,cfg_runPreprocessing,cfg_partAlg,cfg_partDir,cfg_outputAs,cfg_saveTrace,cfg_verbose) = processArgs args
